...
parser.add_argument('--dir_param_file', help='path to param file', type=str, default=None)
...
parser.add_argument('--zsim', help='Simulation snapshot', type=float)
parser.add_argument('--output_dir', help='output directory to save the results', type=str, default='results')
parser.add_argument('--tracer', help='tracer to chose LRG, ELG or QSO', type=str, choices=['ELG', 'LRG', 'QSO'])
...
file_param = args.dir_param_file if args.dir_param_file is not None else f'fit_param_{args.tracer}.yaml'
args_hod = yaml.load(open(file_param, 'r'), Loader=yaml.FullLoader)  

args_hod['hcat']['Abacus']['z_simu'] = args.zsim
args_hod['fit_param']['zmin'], args_hod['fit_param']['zmax'] = [0.4,0.6] if args.zsim == 0.5 else [0.6,0.8] if args.zsim == 0.725 else [0.8,1.1] if args.zsim == 0.95 else [1.1,1.4] if args.zsim == 1.25 else [1.4,1.7] if args.zsim == 1.55 else [1.7,2.1]
...
minimizer_options = {"maxiter":5, "popsize":5000, 'xtol':1e-2, 'workers':mpi_comm.Get_size(),  'backend':'mpi'}
